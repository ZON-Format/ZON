...
def values_equivalent(original, decoded):
    ...
    # Numeric equivalence: 127.0 and 127 are equivalent
...
def check_nested_structure(original, decoded, path='root', allow_numeric_equivalence=True):
    ...
    """
    # Type check with numeric tolerance
    ...
    if type(original) != type(decoded):
        # Special case: int/float are considered compatible
    ...
    if isinstance(original, dict):
        # Check keys match (regardless of order)
...
def main():
    ...
    for dataset in datasets:
        if dataset.name.startswith('internet_'):
            continue  # Skip internet data for now
