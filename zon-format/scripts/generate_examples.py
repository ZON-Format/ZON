...
Port of generate_examples.ts from the TypeScript implementation.
Generates example files in JSON and ZON formats for various data structures.
"""
import os
import sys
import json
...
array_of_primitives = ["apple", "banana", "cherry", "date", "elderberry"]
...
dirty_data = {
    "primitives": {
        ...
        "floats": [0.0, 1.1, -1.1, 3.14159, -2.71828],
        "booleans": [True, False],
        "nulls": [None],
        "strings": [
...
def create_deep_recursion(depth):
    obj = {"end": "bottom"}
    for i in range(depth):
        obj = {"level": i, "next": obj}
    return obj

deep_recursion = create_deep_recursion(50)

# List of examples to generate
examples = [
    ...
    {"name": "09_unified_dataset", "data": unified_dataset, "desc": "Unified Benchmark Dataset"},
...
def main():
    ...
    for ex in examples:
        ...
        json_content = json.dumps(ex['data'], indent=2)
        zon_content = zon.encode(ex['data'])
        ...
        print(f"   JSON: {len(json_content)} bytes")
        print(f"   ZON:  {len(zon_content)} bytes")
        print("---")
